...
LABELS = ['Criticality: zero', 'Criticality: low', 'Criticality: mild', 'Criticality: high']

def get_label(popularity: float) -> str:
    if popularity < 1.0:
        return LABELS[0]
    elif popularity < 10.0:
        return LABELS[1]
    elif popularity < 100.0:
        return LABELS[2]
    else:
        return LABELS[3]
...
def label_translation(translation_pull_request: PullRequest):
    ...
    labels_to_remove = [label.name for label in labels if (label.name.startswith("Criticality:") or label.name.startswith("Predicted:")) and label.name != label_to_add]
